...
import { getDataSource2 } from './getDataSource'
 ...
 *
 * @returns {Promise<any[]>} A promise that resolves to an array of published blog posts.
...
export default async function getPosts(): Promise<any[]> {
  ...
  // const posts = await getDataSource2({
  // dataSourceId: process.env.NOTION_DATA_SOURCE_ID_WRITING ?? '',
  // filter: {
  //   and: [
  //     { property: 'Destination', multi_select: { contains: 'blog' } },
  //     { property: 'Status', status: { equals: 'Published' } },

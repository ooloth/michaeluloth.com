 ...
 */
const REQUIRED_OG_TAGS = ['og:title', 'og:image', 'og:url', 'og:type', 'og:site_name', 'og:locale'] as const

/**
 * Required OpenGraph meta tags for article pages (og:type="article").
 */
const REQUIRED_ARTICLE_TAGS = ['article:published_time', 'article:modified_time', 'article:author'] as const

/**
 * Required Twitter Card meta tags.
 */
const REQUIRED_TWITTER_TAGS = ['twitter:card', 'twitter:creator', 'twitter:title', 'twitter:image'] as const
...
// Only execute validateMetadata() when this file is run directly (not when imported by tests)

 ...
 */
export const REQUIRED_OG_TAGS = ['og:title', 'og:image', 'og:url', 'og:type', 'og:site_name', 'og:locale'] as const

/**
 * Required OpenGraph meta tags for article pages (og:type="article").
 */
export const REQUIRED_ARTICLE_TAGS = ['article:published_time', 'article:modified_time', 'article:author'] as const

/**
 * Required Twitter Card meta tags.
 */
export const REQUIRED_TWITTER_TAGS = ['twitter:card', 'twitter:creator', 'twitter:title', 'twitter:image'] as const

/**
 * Directories to exclude when discovering blog posts.
 * These are not blog post content directories.
 */
export const EXCLUDE_DIRS = new Set(['blog', 'likes', '_next', 'api', '404', '_not-found'])
